  ...
  for (auto& tf : tfs.transforms)
  {
    try
    {
          ...
          (tf.child_frame_id == baseLink || tf.header.frame_id == baseLink))
      {
        ...
        bool valid = mrpt::ros2bridge::lookupSensorPose(
            p, tfBuffer, cli.base_link_frame, odom_frame_id, std::nullopt);
        if (valid)
        {
 ...
 public:
  Transcriber(const mrpt::containers::yaml& config)
  {
    ...
    for (auto& sensorNode : config["sensors"].asMap())
    {
      ...
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToImage(
      ...
      else if (sensorType == "CObservationPointCloud")
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToPointCloud2(
              sensorName, m, *tfBuffer, cli.base_link_frame, fixedSensorPose);
          return Obs(v.begin(), v.end());
        };
        m_lookup[sensor.at("topic").as<std::string>()].emplace_back(callback);
      ...
      else if (sensorType == "CObservation2DRangeScan")
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToLidar2D(
              sensorName, m, *tfBuffer, cli.base_link_frame, fixedSensorPose);
          return Obs(v.begin(), v.end());
        };
        m_lookup[sensor.at("topic").as<std::string>()].emplace_back(callback);
      ...
      else if (sensorType == "CObservationRotatingScan")
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToRotatingScan(
              sensorName, m, *tfBuffer, cli.base_link_frame, fixedSensorPose);
          return Obs(v.begin(), v.end());
        };
        m_lookup[sensor.at("topic").as<std::string>()].emplace_back(callback);
      ...
      else if (sensorType == "CObservationIMU")
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToIMU(
              sensorName, m, *tfBuffer, cli.base_link_frame, fixedSensorPose);
          return Obs(v.begin(), v.end());
        };
        m_lookup[sensor.at("topic").as<std::string>()].emplace_back(callback);
      ...
      else if (sensorType == "CObservationGPS")
      {
        auto callback = [=](const rosbag2_storage::SerializedBagMessage& m) -> Obs
        {
          auto v = mrpt::ros2bridge::rosbag2ToGPS(
              sensorName, m, *tfBuffer, cli.base_link_frame, fixedSensorPose);
          return Obs(v.begin(), v.end());
        };
        m_lookup[sensor.at("topic").as<std::string>()].emplace_back(callback);
